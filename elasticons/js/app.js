...
function updateSvgAnimationStyles() {
  ...
  const isClickMode = clickBtn.checked;
  ...
  allSvgs.forEach(svg => {
    ...
    if (isHoverMode) {
      ...
      const clickRegex = new RegExp(`#${rootId}(?=[\\s{])`, 'g');
      cssText = cssText.replace(clickRegex, `#${rootId}.clicked`);
      
      // Use setTimeout to guarantee a micro-delay so the browser repaints 
      svg.setAttribute('onclick', "let el=this; el.classList.remove('clicked'); setTimeout(() => el.classList.add('clicked'), 10);");
...
window.updateControlStates = function() {
  ...
  const allControls = [strokeSlider, sizeSlider, colorInput, responsiveCheckbox, onloadBtn, hoverBtn, clickBtn, iterationSlider, infiniteCheckbox];
  ...
  try {
    ...
    for (const name of masterList) {
      try {
        ...
        if (window.isPaid) {
          const svgRes = await fetch(`../img/iconsvg/${name}.svg`);
...
function getCleanSvgSource(originalSvg) {
  ...
  // Ensure the onclick attribute & cursor is removed if we are not in Click mode
  if (!clickBtn.checked) {
      svgClone.removeAttribute('onclick');
      svgClone.style.cursor = '';
      if (svgClone.getAttribute('style') === '') svgClone.removeAttribute('style');
  }

  // General cleanups
  if (svgClone.classList.contains('icon-responsive')) svgClone.classList.remove('icon-responsive');
  if (svgClone.classList.contains('clicked')) svgClone.classList.remove('clicked');
